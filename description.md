...
Since we are using JaCoCo to calculate code coverage in this project and Travis CI doesn't support this tool, we have to use an external tool called codecov in order to see the reports of JaCoCo. To use this tool, we must simply sing up in https://codecov.io with our github account and grant access to the repository. Once done, we will have access to all the reports generated by gradle in Travis CI on codecov's page.
